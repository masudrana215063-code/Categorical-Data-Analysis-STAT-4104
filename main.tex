...
\usepackage{tocloft}
\usepackage{inconsolata} % Better monospaced font with bold support
...
\fancyfoot[C]{\color{DeepGold}\thepage}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=north, align=center] at ([yshift=-4cm]current page.north) {
        ...
        {\color{RoyalNavy}\fontsize{32}{36}\selectfont \textbf{CATEGOICAL DATA ANALYSIS}}\\[0.5cm]
        {\color{Charcoal}\Large{COURSE CODE : }STAT4104}\\[0.8cm]
        {\color{DeepGold}\rule{11cm}{1.2pt}}
    };

  % Left Column: Submitted By
\node[anchor=north east, align=left] at ([xshift=-1cm, yshift=-2cm]current page.center) {
    {\large\textbf{\color{RoyalNavy} SUBMITTED BY}}\\[0.4cm]
    \textbf{\color{Charcoal}Name:} Md. Masud Rana\\[0.2cm]
    \textbf{\color{Charcoal}ID:} 12110058 \\[0.2cm]
    \textbf{\color{Charcoal}Reg No:} 000015921 \\[0.2cm]
    \textbf{\color{Charcoal}Session:} 2021-2022 \\[0.2cm]
    \textbf{\color{Charcoal}Year/Sem:} 4\textsuperscript{th} Year, 1\textsuperscript{st} Sem
};

% Right Column: Submitted To
\node[anchor=north west, align=left] at ([xshift=1cm, yshift=-2cm]current page.center) {
    {\large\textbf{\color{RoyalNavy} SUBMITTED TO}}\\[0.4cm]
    \textbf{\color{Charcoal}Name:} Dr. Md. Siddikur Rahman \\[0.2cm]
    \textbf{\color{Charcoal}Title:} Associate Professor \\[0.2cm]
    \textbf{\color{Charcoal}Dept:} Statistics \\[0.2cm]
    \textbf{\color{Charcoal}Inst:} Begum Rokeya University
};

    
    % Footer Date
    \node[anchor=south] at ([yshift=2cm]current page.south) {
        {\color{Charcoal}\large \textsc{Date of Submission: May 19, 2026}}
    };
\end{tikzpicture}
...
\begin{center}
    ...
    \begin{tabular}{p{0.85\textwidth} r}
        ...
        \textbf{\color{RoyalNavy}Problem 7:} Negative Binomial Regression - Days Absent & 18\\
        \textbf{\color{RoyalNavy}Problem 8:} Zero-Inflated Poisson Regression - Fish Catch Data & 19\\
        \textbf{\color{RoyalNavy}Problem 9:} Zero-Inflated Poisson - Detailed Analysis & 21\\
        \textbf{\color{RoyalNavy}Problem 10:} Zero-Truncated Poisson - Hospital Length of Stay & 23\\
        \textbf{\color{RoyalNavy}Problem 11:} Zero-Truncated Negative Binomial - Hospital Stay & 25\\
    \end{tabular}
    \vspace{0.5cm}
    \end{tcolorbox}
\end{center}
...
\end{verbatim}
\end{outputbox}
\section{interpretation}The logistic regression analysis shows that:

\begin{itemize}
    \item Higher GRE scores significantly improve the chance of admission. 
    \item Higher GPA significantly increases the likelihood of admission. 
    \item Students graduating from lower prestige institutions (rank 2, 3, and 4) have significantly lower admission probabilities compared to students from rank 1 institutions. 
\end{itemize}
...
\begin{itemize}
    \item Math score is a strong positive predictor of the number of awards earned. 
    \item Students in Program Type 2 earn significantly more awards than students in the reference program category. 
    \item Program Type 3 does not significantly differ from the reference category. 
\end{itemize}
...
\section{Problem Statement}
School administrators study the attendance behavior of high school juniors at two schools. Predictors of the number of days of absence include the type of program in which the student is enrolled and a standardized test in math. The data set is taken from \url{https://stats.idre.ucla.edu/stat/stata/dae/nb_data.dta} and fit the negative binomial generalized linear models (GLMs) to identify the factors associated with number of awards earned by students at one high school. Interpret the result.
...
\begin{itemize}
    \item Age does not significantly affect the length of hospital stay. 
    \item HMO insurance may slightly reduce hospital stay duration, but the effect is not statistically significant. 
    \item Death status is a significant predictor, with patients who died having shorter hospital stays on average. 
\end{itemize}
